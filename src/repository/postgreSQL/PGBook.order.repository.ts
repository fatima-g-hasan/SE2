  ...
  implements IRepository<IdentifiableBook>, Initializable
{
  ...
  private mapper = new SQLBookMapper();
  ...
  async init(): Promise<void> {
    try {
      ...
      logger.info("Book table initialized (Neon PostgreSQL)");
    } catch (error: unknown) {
      logger.error("Failed to initialize Book table", error as Error);
      throw new InitializationException(
        "Failed to initialize Book table",
        error as Error
      );
    }
  }
  ...
  async get(id: id): Promise<IdentifiableBook> {
    try {
      ...
      const rows = result as unknown as BookRow[];
  ...
  async getAll(): Promise<IdentifiableBook[]> {
    try {
      ...
      return rows.map((book) => this.mapper.map(book));
    } catch (error: unknown) {
  ...
  async update(item: IdentifiableBook): Promise<void> {
    try {
      ...
      logger.error(
        "Failed to update book of id %s %o",
        item.getId(),
        error as Error
      );
  ...
  async delete(id: id): Promise<void> {
    try {
      ...
      logger.error(
        "Failed to delete book of id %s %o",
        id,
        error as Error
      );

      throw new DbException(
        `Failed to delete book of id ${id}`,
        error as Error
      );
    }
  }
